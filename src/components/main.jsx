...
function HotelCard({ hotel }) {
  ...
  const bestRate = hotel.prices.reduce((best, rate) => (rate.price < best.price ? rate : best));
      ...
      <div className="hotel-info">
        <div>
          <div className="hotel-title">
            ...
            <button className="icon-button" aria-label={`Save ${hotel.name}`}>
              <Heart size={18} />
            </button>
          </div>
          <div className="meta">
...
function App() {
  ...
  const visibleHotels = useMemo(
    () => hotelResults.filter((hotel) => hotel.prices.some((rate) => rate.price <= maxPrice)),
  ...
  async function handleSearch(event) {
    ...
    try {
      ...
      setHotelResults(results.length ? results : getDestinationFallback(searchParams.destination));
      setStatus(results.length ? 'Showing live backend results' : 'Showing local destination results');
    } catch (error) {
  ...
  return (
    <div className="app-shell">
      ...
      <main className="content">
        {/* View Mode Tabs */}
        <div className="flex gap-2 mb-6 border-b border-gray-200 px-4">
          <button
